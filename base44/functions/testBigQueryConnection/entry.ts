...
Deno.serve(async (req) => {
  try {
    ...
    const sql = `SELECT COUNT(*) as row_count FROM \`${project_id}.${dataset}.${table_name}\` LIMIT 1`;
    const bqRes = await fetch(`https://bigquery.googleapis.com/bigquery/v2/projects/${project_id}/queries`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${accessToken}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ query: sql, useLegacySql: false, maxResults: 1 }),
    });
    const bqBody = await bqRes.json();
    if (!bqRes.ok || bqBody.error) throw new Error(bqBody.error?.message || JSON.stringify(bqBody.error));

    const rowCount = Number(bqBody.rows?.[0]?.f?.[0]?.v ?? 0);
    return Response.json({ success: true, project: project_id, dataset, table: table_name, row_count: rowCount });
